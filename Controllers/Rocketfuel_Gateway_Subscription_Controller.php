...
class Rocketfuel_Gateway_Controller extends \WC_Payment_Gateway
{
	...
	file_put_contents(__DIR__.'/error.json',json_encode( $error),FILE_APPEND);
		}
		$merchant_cred = array(
			'email' => $this->email,
		...
		file_put_contents(__DIR__.'/shipping.json',json_encode($shippingAddress),FILE_APPEND);
if($shippingAddress){
	$data['body']['shippingAddress'] = $shippingAddress ;
}
